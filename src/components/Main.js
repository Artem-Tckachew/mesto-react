...
function Main({cards, onEditProfile, onAddPlace, onEditAvatar, onCardClick, onCardLike, onCardDelete}) {
  const currentUsers = React.useContext(currentUser);
  return (
    <main className="content">
      <section className="profile">
        <div className="profile__avatar" name="avatar"  style={{ backgroundImage: `url(${currentUsers.avatar})` }} >
          <div className="profile__avatar profile__avatar_hover" onClick={onEditAvatar} />
        </div>
        <div className="profile__intro">
          <h2 className="profile__title" name="name">{currentUsers.name}</h2>
          <p className="profile__subtitle" name="about">{currentUsers.about}</p>
